...
class ServerSocket
{
	...
	public:
		...
		bool isMethodAllowed(const std::string &method, int locationIndex);
		int getReturnCode(int locationIndex);
		bool isLocationDenied(int locationIndex);
		bool doesLocationMatch(const std::string &path, int locationIndex);
		std::map<int, std::string> parseFileInfo(std::string path);
		std::map<int, std::string> checkForRedirects(std::string path);
		void normalizePath(std::string &path);
		void prependWebRoot(std::string &path);
		void appendDefaultFileIfRoot(std::string &path, int &trigger);
		void handleDirectoryOrFile(std::string &path, int &trigger);
		std::map<int, std::string> generateResponse(std::string path, int trigger);
